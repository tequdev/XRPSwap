const parseLocaleString = (integer: string) => {
  return parseFloat(integer).toLocaleString()
}
export const significantDigits = (value: number, n: number) => {
  ...
  if (value >= 1) {
    ...
    if (splitted.length === 1) {
      if (integer.length > n - 1) return parseLocaleString(integer) // 1234→'1234'
      return parseLocaleString(integer) + '.' + '0'.repeat(n - integer.length) // 123→123.0
    }
    if (integer.length > n - 1) return parseLocaleString(integer) // 1234.56→1234
